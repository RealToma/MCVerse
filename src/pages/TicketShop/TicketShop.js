...
import { ticketAddress, mcvAddress, ticketMintAddress } from 'contracts/address'
...
const TicketShop = () => {
  ...
  const init = async (ticketMintContract, ticketContract, address) => {
    try {
      ...
      console.log('ticket: ', mintPrice.toString(), ended, totalMinted.toString())
  ...
  const handleMint = useCallback(async () => {
    try {
      ...
      const mcvAllowance = await mcvContract.allowance(address, ticketMintAddress)
      if (parseInt(ethers.utils.formatEther(mcvAllowance)) < mintPrice * 10) {
        const res = await mcvContract.approve(ticketMintAddress, ethers.utils.parseEther(mintPrice * 10 + ''))
        await res.wait()
      }
  ...
  useEffect(() => {
    ...
    const ticketMintContract = new ethers.Contract(ticketMintAddress, mintABI, signer)
    ...
    <PageMainWrapper>
      ...
      <div className="relative pb-4 mb-10 md:pb-40 md:mb-24 2xl:pb-20 2xl:mb-12 callout-bg">
        <div className="absolute left-0 top-0 w-16 h-16 rounded-full bg-black border border-[#009EFF] shadow-[0_0_9px_rgba(77,201,255,0.9)] hidden md:flex items-center justify-center">
          <img src={Images.imgTicket} className="" alt="" />
        </div>
        <div className="hidden ml-1 sm:ml-6 md:ml-20 md:flex md:items-center md:justify-between md:mr-[14vw] h-16">
          <div className="md:flex md:items-center">
            ...
            >
              MINT AN ADMISSION TICKET TO THE CAR SHOW
            </div>
            ...
            <div className="text-white text-[10px] font-medium tracking-[2px] text-shadow-blue">
              Ticket is required to enter the MCVerse Car Show Mission.
            </div>
          </div>
        </div>
        <div className="relative grid gap-2 mt-2 xl:pr-4 xl:grid-cols-2 2xl:grid-cols-7 3xl:grid-cols-3">
          ...
          <div className="2xl:col-span-3 3xl:col-span-1 bg-[#000F44] bg-opacity-60 border-[0.5px] border-[#80C9FF] pl-6 pr-4 py-6 md:ml-4 2xl:ml-0 rounded-t-2xl 2xl:rounded-tr-md rounded-b-2xl">
            <div className="flex items-center gap-4 mx-auto md:mx-0">
              <img src={Images.logoTitle} alt="logo" className="w-80" />
              <img src={Images.imgTitleCarShow} alt="title" />
            </div>
            <div className="py-5 text-sm leading-5 ">
              Welcome to the MCVerse Box Office! Mint a ticket to get admission into the MCVerse Car Show, where you can win prizes for best
              in show or ugliest car of the year! Tickets cannot be refunded if unused. Tickets expire when Car Show ends, and are not
              applicable towards another mission or event.
            </div>
            ...
            <div className="grid grid-cols-none gap-2 py-2 sm:grid-cols-2">
              ...
              <div>
                <div className="pb-2">
                  <div className=" text-[10px] leading-3 text-[#EBBB00] tracking-[1px] text-shadow-blue uppercase">tickets minted</div>
          ...
          <div className="2xl:col-span-4 3xl:col-span-2 bg-[#000F44] bg-opacity-60 border-[0.5px] border-[#80C9FF] rounded-xl md:ml-4 3xl:ml-0 p-6">
            <div className=" text-lg leading-5 text-[#21D0FF]">How many tickets would you like to mint?</div>
                  ...
                  <div key={index} className="flex items-center justify-center w-full">
                    ...
                    >
                      {index <= numBoxToBuy ? (
                        ...
                        >
                          <img src={Images.imgTicket} alt="ticket" loading="lazy" className="w-full" />
                        </div>
                      ) : (
                        <div
            ...
            <div className=" text-sm leading-5 text-[#FFCE0D] py-0.5">{numBoxToBuy} Tickets Selected</div>
            <div className="border-b border-dashed border-[#2EA0DC] my-2"></div>
            <>
              ...
              <div className="flex flex-wrap gap-10 py-3">
                ...
                <div>
                  <button
                    ...
                    onClick={() => handleMint()}
                  >
                    {minting ? <PulseLoader color="#21D0FF" size={10} className="px-6" /> : <>mint my tickets</>}
                  </button>
                </div>
              </div>
            </>
